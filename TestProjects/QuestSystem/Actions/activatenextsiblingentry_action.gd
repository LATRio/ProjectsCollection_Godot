...
func execute(caller_id: int) -> void:
	...
	if entry:
		...
		if sibling_id == -1:
			push_error("[QuestSystem] Caller Entry ID [{0}] doesn't have any siblings!", caller_id)
			return
		QuestSystem.set_entry_status(sibling_id, QuestSystem.EntryStatus.INPROGRESS)
	else:
		push_error("[QuestSystem] Failed to execute [ActivateNextSiblingEntry_Action] called by Entry ID [{0}] caller didn't exist in database!", caller_id)
